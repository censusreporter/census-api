...
def geo_search():
    ...
    if with_geom:
        g.cur.execute("SELECT sumlevel,geoid,name,ST_AsGeoJSON(ST_Simplify(the_geom,0.7)) as geom FROM tiger2012.census_names_simple WHERE %s ORDER BY sumlevel, aland DESC LIMIT 5;" % where, where_args)
    else:
